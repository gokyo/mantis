...
object BlockImporter {
  ...
  sealed trait BlockImportType {
    def recordMetric(time: Long): Unit
  }

  case object MinedBlockImport extends BlockImportType {
    override def recordMetric(time: Long): Unit = RegularSyncMetrics.recordMinedBlockPropagationTimer(time)
  }
  ...
  case object NewBlockImport extends BlockImportType {
    override def recordMetric(time: Long): Unit = RegularSyncMetrics.recordImportNewBlockPropagationTimer(time)
  }
